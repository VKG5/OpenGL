...
// Global variables to store the camera position and rotation
glm::vec3 cameraPosition = glm::vec3(0.0f, 0.0f, 0.0f);
glm::quat cameraRotation = glm::quat();
...
int main( void )
{
    ...
    glfwSetKeyCallback(
        ...
        if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
            cameraPosition += cameraRotation * glm::vec3(0.0f, 0.0f, 1.0f);
        } else if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
            cameraPosition -= cameraRotation * glm::vec3(0.0f, 0.0f, 1.0f);
        }

        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
            cameraPosition -= cameraRotation * glm::vec3(1.0f, 0.0f, 0.0f);
        ...
        // Calculate the camera rotation based on the mouse cursor movement
        float mouseSensitivity = 1.0f;
        float deltaX = xpos - cameraRotation.x;
        float deltaY = ypos - cameraRotation.y;
        cameraRotation = glm::quat(1.0f, deltaX * mouseSensitivity, deltaY * mouseSensitivity, 0.0f) * cameraRotation;
        }
    );
	...
	const aiScene* scene = aiImportFile( "D:/Programs/C++/Computer_Graphics_TCD/files/untitled.gltf", aiProcess_Triangulate );
	...
	while ( !glfwWindowShouldClose( window ) )
	{
        ...
        mat4 projectionMatrix = glm::perspective(45.0f, ratio, 0.1f, 1000.0f);
        mat4 viewMatrix = glm::lookAt(cameraPosition, cameraPosition + cameraRotation * glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
