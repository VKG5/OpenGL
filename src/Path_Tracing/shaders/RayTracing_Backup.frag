...
// Uniform to toggle Blinn-Phong shading
...
vec3 blinnPhongShading(SurfacePoint point, vec3 viewDir) {
    // Basic ambient color
    vec3 ambientColor = vec3(0.5) * point.material.albedo;
    ...
    for (int lightIndex = 0; lightIndex < u_lights.length(); lightIndex++) {
        PointLight light = u_lights[lightIndex];
        lightDir = normalize(light.position - point.position);
        halfVec = normalize(lightDir + viewDir);

        // Diffuse component
        float diff = max(dot(point.normal, lightDir), 0.0);
        diffuseColor += diff * light.color * point.material.albedo;

        // Specular component
        specAngle = max(dot(point.normal, halfVec), 0.0);
        specularColor += pow(specAngle, point.material.specularExponent) * light.color * point.material.specular;
    }

    return ambientColor + (diffuseColor + specularColor) / float(u_lights.length());
}
...
void main() {
    ...
    // Use Blinn Phong
        ...
        if (raycast(cameraRay, hitPoint)) {
            vec3 viewDir = normalize(u_cameraPosition - hitPoint.position);
            vec3 color = blinnPhongShading(hitPoint, viewDir);
