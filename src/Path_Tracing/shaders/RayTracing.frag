...
struct PointLight {
    ...
    // Only points within the reach of the light will be affected
	float reach;
};
...
// How many passes have been added to the texture
uniform int u_accumulatedPasses;

// If this is true, the shader will draw the input texture directly to the screen. (Used to draw the contents of the FBO to the screen)
...
// Index of the selected object for highlighting
uniform int u_selectedSphereIndex;

// Uniform to toggle Simple shading & Ray Tracing
uniform int u_useBlinnPhong;

float rand(vec2 co){
    // Magic Numbers to randomize noise generator
    vec2 newMagic = vec2(14.4527, 76.8761);

    float newScale = 39282.6275;

    float dotProduct = dot(co, newMagic);

    float scaledSine = sin(dotProduct) * newScale;

    // Return the fractional part of the scaled sine value - [0, 1]
    return fract(scaledSine);
}
...
{
    // *Reference: https://gist.github.com/Shtille/1f98c649abeeb7a18c5a56696546d3cf
...
// *Reference: https://bitbucket.org/Daerst/gpu-ray-tracing-in-unity/src/Tutorial_Pt2/Assets/RayTracingShader.compute
...
// Sample the skybox colors for reflections
...
// *Reference: https://github.com/SebLague/Ray-Tracing
// The shader code written by Sebastian Lague for Unity has been heavily translated for my application
	...
	for (int depth = 0; depth < u_lightBounces; depth++) {
		...
		if (raycast(Ray(rayOrigin, rayDirection), hitPoint)) {
            ...
            if (roulette < specChance) {
				...
				// This means both the hit material's albedo and specular are totally black, so there won't be anymore light
                // We can stop calculations here
				break;
			}
		}
...
// Selecting an object
void highlightSelectedObject(vec2 centeredUV, vec3 rayDir, Ray cameraRay) {
    // Selected Object outline render
...
void calculateRayTracing(vec2 centeredUV, vec3 rayDir, Ray cameraRay) {
    if (u_directOutputPass) {
		fragColor = texture(u_screenTexture, fragPos);
		...
		if (u_blur > 0.0 && u_accumulatedPasses > 0) {
            centeredUV += vec2(rand(vec2(1, u_time)+fragPos.xy)*u_blur-u_blur/2, rand(vec2(2, u_time)+fragPos.yx)*u_blur-u_blur/2);
        }
		...
		if (u_accumulatedPasses > 0) {
			// Bloom
			SurfacePoint hitPoint;
			vec3 offsetDirection = cameraRay.direction + vec3(rand(vec2(1, u_time)+fragPos)*u_bloomRadius-u_bloomRadius/2, rand(vec2(2, u_time)+fragPos)*u_bloomRadius-u_bloomRadius/2, rand(vec2(3, u_time)+fragPos)*u_bloomRadius-u_bloomRadius/2);
...
// This is for previewing the objects
vec4 simpleShading(SurfacePoint point) {
    vec3 viewDirection = normalize(u_cameraPosition - point.position);
    vec4 objCol = vec4(0.0, 0.0, 0.0, 0.0);

    // Controls the width of the outline
    float edgeWidth = 0.15;
    float facingRatio = dot(viewDirection, point.normal);
    bool isEdge = facingRatio < edgeWidth;

    if (isEdge) {
        objCol = vec4(0.0, 0.0, 0.0, 1.0);
    }

    else {
        objCol = vec4(point.material.albedo, 1.0);
    }
...
void main() {
	vec2 centeredUV = (fragPos * 2 - vec2(1)) * vec2(u_aspectRatio, 1.0);
    vec3 rayDir = (normalize(vec4(centeredUV, -1.0, 0.0)) * u_rotationMatrix).xyz;
    Ray cameraRay = Ray(u_cameraPosition, rayDir);

    // Use Ray Tracing
    if(u_useBlinnPhong > 0) {
        calculateRayTracing(centeredUV, rayDir, cameraRay);
    }

    // Use Simple Shading for viewing
    else {
        SurfacePoint hitPoint;
        if (raycast(cameraRay, hitPoint)) {
            fragColor = simpleShading(hitPoint);
        }

        else {
             // Background color
            fragColor = vec4(61.0 / 255.0, 61.0 / 255.0, 61.0 / 255.0, 1.0);
        }
    }

    // Selected object outline rendering
    highlightSelectedObject(centeredUV, rayDir, cameraRay);
}
