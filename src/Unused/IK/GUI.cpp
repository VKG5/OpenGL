...
void GUI::elements(const std::string& shadingMode) {
    ...
    if (ImGui::BeginTabBar("Properties"))
    {
        ...
        if (ImGui::BeginTabItem("IK")) {
            ...
            ImGui::Checkbox("2D", &isTwoDIK);
            ImGui::DragInt("Chain Length", (int*)&chainLength, 1, 1, 8);
            ImGui::DragInt("Max Iterations", (int*)&maxIterations);
            ImGui::DragFloat("Tolerance", (float*)&tolerance, sliderSpeed, 0.0001f);
            ImGui::DragFloat("Epsilon", (float*)&epsilon, sliderSpeed, 0.0f);
            ImGui::DragFloat3("Target Position", targetLocation, sliderSpeed);
            ImGui::DragFloat3("Target End Position", targetEndLocation, sliderSpeed);
            ImGui::DragFloat("Target Speed", (float*)&targetSpeed, sliderSpeed);

            ImGui::Spacing();
            ImGui::DragInt("Interpolation Type", (int*)&interpolationType, 1, 1, 2);
            ImGui::DragFloat("Animation Rate", (float*)&animationRate, sliderSpeed);
...
void GUI::setTargetLocation(float x, float y, float z) {
    targetLocation[0] = x;
    targetLocation[1] = y;
    targetLocation[2] = z;
}

void GUI::setIsAnimate(bool flag) {
    isAnimate = flag;
}

void GUI::setIsIK(bool flag) {
    isIK = flag;
}
